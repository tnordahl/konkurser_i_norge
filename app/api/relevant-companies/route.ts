...
export async function GET(request: NextRequest) {
  try {
    ...
    [...companiesWithAddressHistory, ...companiesWithRiskAlerts, ...bankruptCompanies, ...highRiskCompanies]
      .forEach(company => {
    ...
    relevantCompanies.sort((a, b) => {
      const scoreA = a.riskScore + (a.addressHistory?.length || 0) * 10 + (a.riskAlerts?.length || 0) * 5;
      const scoreB = b.riskScore + (b.addressHistory?.length || 0) * 10 + (b.riskAlerts?.length || 0) * 5;
    ...
    const stats = {
      ...
      totalActiveRiskAlerts: await prisma.companyRiskAlert.count({ where: { isActive: true } })
    };
    ...
    const formattedCompanies = relevantCompanies.map(company => ({
      ...
      relevanceIndicators: {
        ...
        isHighRisk: company.riskScore >= 60,
        isBankrupt: company.isBankrupt
      },
      addressHistory: company.addressHistory?.slice(0, 3).map(addr => ({
    ...
    return NextResponse.json({
      ...
      explanation: {
        purpose: "This table contains only companies with RELEVANT data for fraud detection",
        relevanceCriteria: [
        ...
        dataQuality: "All companies in this table have at least one fraud-relevant indicator"
      },
      metadata: {
