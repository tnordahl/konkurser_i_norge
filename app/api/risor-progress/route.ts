...
function logProgress(step: string, status: ProgressUpdate["status"], details: string, data?: any) {
  ...
  console.log(`📊 [${update.timestamp.split('T')[1].split('.')[0]}] ${step}: ${details}`);
  if (data) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    logProgress("API_FETCH", "completed", `✅ Successfully fetched all entities from API`, { 
    ...
    logProgress("DATA_ANALYSIS", "starting", "🔍 Analyzing data structure and quality...");
    
    const analysis = analyzeEntities(entities);
    
    logProgress("DATA_ANALYSIS", "completed", "✅ Data analysis complete", analysis);

    // Step 3: Prepare data for saving (simulate processing)
    logProgress("DATA_PREPARATION", "starting", "⚙️ Preparing data for database storage...");
    
    const preparedData = await prepareDataWithProgress(entities);
    
    logProgress("DATA_PREPARATION", "completed", "✅ Data preparation complete", {
    ...
    logProgress("COMPLETION", "completed", `🎉 Risör data collection completed successfully in ${Math.round(totalTime/1000)}s`);
...
async function fetchAllRisorEntitiesWithProgress() {
  ...
  logProgress("API_FETCH", "in_progress", "🌐 Starting API requests to Brønnøysundregistrene...");
  ...
  while (true) {
    ...
    if (!response.ok) {
      logProgress("API_FETCH", "error", `❌ API request failed: ${response.status} ${response.statusText}`);
    ...
    if (entities.length === 0) {
      logProgress("API_FETCH", "in_progress", "🏁 No more entities found, stopping pagination");
      break;
    }
    ...
    if (page === 0 && entities.length > 0) {
      ...
      logProgress("API_FETCH", "in_progress", "📋 Sample entity from first page", {
        name: sample.navn,
    ...
    if (page >= totalPages - 1 || pageSize * (page + 2) > 10000) {
      logProgress("API_FETCH", "in_progress", `🛑 Reached API limit or end of data`, {
    ...
    if (page < totalPages) {
      logProgress("API_FETCH", "in_progress", "⏳ Rate limiting: waiting 1 second before next page...");
...
function analyzeEntities(entities: any[]) {
  logProgress("DATA_ANALYSIS", "in_progress", "📊 Analyzing entity data quality...");
  ...
  entities.forEach(entity => {
    ...
    if (entity.organisasjonsform?.kode) analysis.organizationForms.add(entity.organisasjonsform.kode);
    if (entity.forretningsadresse?.postnummer) analysis.postalCodes.add(entity.forretningsadresse.postnummer);
    if (entity.forretningsadresse?.poststed) analysis.cities.add(entity.forretningsadresse.poststed);
    if (entity.naeringskode1?.beskrivelse) analysis.industries.add(entity.naeringskode1.beskrivelse);
    
    if (entity.registreringsdatoEnhetsregisteret) {
      const year = new Date(entity.registreringsdatoEnhetsregisteret).getFullYear();
  ...
  return {
    ...
    registrationYearRange: analysis.registrationYears.size > 0 
      ? `${Math.min(...analysis.registrationYears)}-${Math.max(...analysis.registrationYears)}`
      : "unknown",
    dataQualityScore: Math.round((analysis.entitiesWithBusinessAddress / analysis.totalEntities) * 100)
  };
}
...
async function prepareDataWithProgress(entities: any[]) {
  ...
  logProgress("DATA_PREPARATION", "in_progress", "🔄 Processing entities in batches...");
  ...
  for (let i = 0; i < entities.length; i += batchSize) {
    ...
    logProgress("DATA_PREPARATION", "in_progress", `⚙️ Processing batch ${batchNumber}/${totalBatches}`, {
    ...
    for (const entity of batch) {
      ...
      if (entity.postadresse && 
          JSON.stringify(entity.postadresse) !== JSON.stringify(entity.forretningsadresse)) {
        addressHistory.push({
          organizationNumber: entity.organisasjonsnummer,
      ...
      if (entity.forretningsadresse?.postnummer) {
        postalCodesSet.add(`${entity.forretningsadresse.postnummer}:${entity.forretningsadresse.poststed}`);
      }
      if (entity.postadresse?.postnummer && 
          entity.postadresse.postnummer !== entity.forretningsadresse?.postnummer) {
        postalCodesSet.add(`${entity.postadresse.postnummer}:${entity.postadresse.poststed}`);
      }
    }
  }
...
export async function GET(request: NextRequest) {
  return NextResponse.json({
    success: true,
    test: "Risör Progress Test",
    description: "Complete data collection for Risör with detailed progress tracking",
