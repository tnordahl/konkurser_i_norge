...
export function startDailyUpdateScheduler() {
  ...
  scheduledTask = cron.schedule('0 2 * * *', async () => {
    console.log('Starting scheduled daily update at', new Date().toISOString())
  ...
  scheduledTask.start()
  console.log('Daily update scheduler started - will run every day at 2:00 AM (Norwegian time)')
}
...
export function getSchedulerStatus() {
  return {
    isRunning: scheduledTask !== null && scheduledTask.getStatus() === 'scheduled',
...
export function initializeScheduler() {
  ...
  if (shouldStartScheduler) {
    ...
    console.log('Scheduler disabled - set ENABLE_SCHEDULER=true to enable in development')
  }
}
