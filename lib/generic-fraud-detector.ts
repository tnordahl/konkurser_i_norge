...
const KNOWN_ESCAPED_COMPANIES = [
  {
    ...
    orgNumber: "989213598",
    evidence: ["Uses RISØR REGNSKAP AS", "Board chairman is lawyer", "User confirmed move from Risør"],
  },
  {
    name: "STRANDGATA 23 RESTAURANTHUS AS",
    orgNumber: "UNKNOWN", // To be investigated
    evidence: ["User reported as escaped from Risør", "Restaurant business - cash intensive"],
  },
  {
    name: "ØSTNES AS", 
    orgNumber: "UNKNOWN", // To be investigated
  ...
  },
  {
    ...
    orgNumber: "UNKNOWN", // To be investigated  
    evidence: ["User reported as escaped from Risør", "Construction/handwerk - high fraud risk"],
  },
];
...
export async function detectEscapedCompanies(
  ...
  console.log(`🔍 GENERIC FRAUD DETECTION: Analyzing escaped companies from kommune ${targetKommuneNumber}`);
  
  const escapedCompanies: EscapedCompanyPattern[] = [];
  
  // Method 1: Professional Service Network Analysis
  const serviceNetworkCompanies = await detectViaServiceNetworks(targetKommuneNumber);
  escapedCompanies.push(...serviceNetworkCompanies);
  
  // Method 2: Address Pattern Analysis
  const addressPatternCompanies = await detectViaAddressPatterns(targetKommuneNumber);
  ...
  const registrationPatternCompanies = await detectViaRegistrationPatterns(targetKommuneNumber);
  ...
  console.log(`✅ Generic detection complete: ${sortedCompanies.length} escaped companies found`);
 ...
 */
async function detectViaServiceNetworks(kommuneNumber: string): Promise<EscapedCompanyPattern[]> {
 ...
 */
async function detectViaAddressPatterns(kommuneNumber: string): Promise<EscapedCompanyPattern[]> {
  const companies: EscapedCompanyPattern[] = [];
 ...
 */
async function integrateKnownCases(kommuneNumber: string): Promise<EscapedCompanyPattern[]> {
  const companies: EscapedCompanyPattern[] = [];
  ...
  if (kommuneNumber === "4201") { // Risør
    ...
    const suspectedCompanies = [
      {
        name: "STRANDGATA 23 RESTAURANTHUS AS",
        indicators: ["RESTAURANT_CASH_BUSINESS", "USER_INTELLIGENCE", "STRANDGATA_ADDRESS_PATTERN"],
      ...
      {
        name: "MINDE HÅNDVERKSTJENESTER AS", 
        indicators: ["CONSTRUCTION_HANDWERK", "HIGH_FRAUD_INDUSTRY", "USER_INTELLIGENCE"],
        riskLevel: "HIGH" as const,
        confidence: 85,
      },
    ];
 ...
 */
async function detectViaRegistrationPatterns(kommuneNumber: string): Promise<EscapedCompanyPattern[]> {
  const companies: EscapedCompanyPattern[] = [];
 ...
 */
function deduplicateCompanies(companies: EscapedCompanyPattern[]): EscapedCompanyPattern[] {
  const seen = new Set<string>();
  const unique: EscapedCompanyPattern[] = [];
  
  for (const company of companies) {
    const key = company.organizationNumber !== "INVESTIGATION_NEEDED" 
 ...
 */
export function calculateKommuneFraudRisk(escapedCompanies: EscapedCompanyPattern[]): {
  riskLevel: "LOW" | "MEDIUM" | "HIGH" | "CRITICAL";
  totalCases: number;
  ...
  const criticalCases = escapedCompanies.filter(c => c.riskLevel === "CRITICAL").length;
  const highCases = escapedCompanies.filter(c => c.riskLevel === "HIGH").length;
  const highConfidenceCases = escapedCompanies.filter(c => c.confidence >= 80).length;
  ...
  if (highConfidenceCases >= 2) {
    ...
    recommendations.push("📊 Implement enhanced monitoring for professional services");
  }
  if (escapedCompanies.length >= 3) {
    recommendations.push("⚠️ Pattern detected: Systematic fraud investigation recommended");
    recommendations.push("🏛️ Review municipality's business oversight procedures");
  }
